...
\ed{Are these super-optimizers too?}
...
In theory, the model checker should be able to accurately check for correctness as long as the number of loop iterations stays below the configured maximum bound of \ed{fill this in please Alex}, and the process does not timeout by taking over \ed{N seconds}.
...
\begin{table}[t]
\caption{Proportion of programs that were reported to outperform the GCC -Og baseline, and via manual evaluation, verified correct. The programs chosen via testcase-only method are denoted by \textit{TC} and programs chosen via the theorem prover method are denoted by \textit{Z3}}
\label{tab:manual}
\vskip 0.15in
\begin{center}
\begin{small}
...
\begin{tabular}{lcc}
\toprule
Task & Verified Correct \\
\midrule
TC Train Pre-Trained TC    & --.--\% \\
TC Test Hill-Climbing TC  & --.--\% \\
TC Train Pre-Trained Z3TC   & --.--\% \\
TC Test Hill-Climbing Z3  & --.-- \% \\
Z3 Train Hill-Climbing Z3  & --.--\% \\
Z3 Test Hill-Climbing Z3  & --.--\% \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
